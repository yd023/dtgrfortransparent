...
def main(args):
    ...
    if args.mode == 1:
        ...
        file_loss = open(os.path.join(dir_out, 'iterations', 'param_loss.log'), 'w')
        file_param = open(os.path.join(dir_out, 'iterations', 'iter_param.log'), 'w')
        num_pyramid_lvl = 9
        weight_pyramid = 4
        times = []
        
        one = torch.tensor([1.0], dtype=torch.float, requires_grad=True)
        ...
        for t in range(opt_spec['num_iters']):
            ...
            imgs = reorg_images(dtgr_render.render(
                args.scene_name, 
            ...
            file_loss.write("%.5e, %.5e, %.5e, %.2e\n" %
                            (param, opt_loss, param_loss, times[-1]))
            file_loss.flush()
