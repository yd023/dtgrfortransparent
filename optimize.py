...
def total_variation_loss_of_parameter(param):
    h_img, w_img = param.size()
    tv_h = torch.pow(param[1:,:]-param[:-1,:], 2).sum()
    tv_w = torch.pow(param[:,1:]-param[:,:-1], 2).sum()
    return (tv_h+tv_w)/(h_img*w_img)

def create_output_directory(dir_out,outdirname):
    if not os.path.exists(dir_out):
        os.mkdir(dir_out)
    if not os.path.exists(os.path.join(dir_out, outdirname)):
        os.mkdir(os.path.join(dir_out, outdirname))
...
def main(args):
    ...
    crspp = 10
    patience = 100
    earlystopping = es.EarlyStopping(patience=patience, verbose=True) 
    ...
    outdirname = f'iterations_lr{lr}_spp_dinamic_{patience}_tv_{tv_lam}_firstspp{spp}_changerateofspp_{crspp}'

    suffix = 2
    while(os.path.exists(os.path.join(dir_out, outdirname))):
        outdirname = f'iterations_lr{lr}_spp_dinamic_{patience}_tv_{tv_lam}_firstspp{spp}_changerateofspp_{crspp}({suffix})'
        suffix += 1

    create_output_directory(dir_out,outdirname)
    ...
    if args.mode == 1:
        ...
        if opt_spec['gen_ref']:
            imgs_ref = dtgr_render.render(
                args.scene_name, args.integrator_name, param_mode='target', component='ref', seed = 1)
        ...
        file_loss = open(os.path.join(dir_out, outdirname, 'iter_loss.log'), 'w')
        file_param = open(os.path.join(dir_out, outdirname, 'iter_param.log'), 'w')
        num_pyramid_lvl = 9
        weight_pyramid = 4
        times = []
        ...
        for t in range(opt_spec['num_iters']):
            ...
            dtgr_render.imwrite(imgs, args.scene_name, args.integrator_name, f'{outdirname}imgs/imgs_iter_{t}', nder_to_output=[0])
            dtgr_render.exr2png(args.scene_name, args.integrator_name, f'{outdirname}imgs/imgs_iter_{t}', nder_to_output=[0])
            
            imgs = reorg_images(imgs)
            
            print(imgs.size())
            
            #dtgr_render.imwrite(imgs, args.scene_name, args.integrator_name, f'imgs_{t}', nder_to_output=[0])
            #dtgr_render.exr2png(args.scene_name, args.integrator_name, f'imgs_{t}', nder_to_output=[0])
            ...
            print(imgs_iter.grad_fn)
            
            print(imgs_iter.size())
            print(imgs_ref.size())

            # compute losses
            img_loss = (imgs_iter - imgs_ref).pow(2).mean()
            mse_loss = torch.sqrt(img_loss)
            param_loss = (param - opt_spec['param_target']).pow(2).sum().sqrt()

            param_detached = param.detach()
            param = param.reshape(11,11)

            tv_loss = total_variation_loss_of_parameter(param)

            param = param.flatten()

            opt_loss = (1 - tv_lam) * mse_loss + tv_lam * tv_loss
            print(opt_loss)

            # write param values
            file_param.write("%d" % t)
            for i in range(vredner.nder):
                file_param.write(", %.5e" % param_detached[i])
            file_param.write("\n")
            file_param.flush()
            ...
            if args.plot:
                # image loss
                loss_record[0].append(opt_loss.detach().numpy())
            ...
            print('render time: %s; elapsed time: %s; opt. loss: %.3e;' %
                  (time_to_string(times[-1]), time_to_string(elapsed_time), opt_loss,))

            # write image/param loss
            file_loss.write("%d, %.5e, %.5e, %.5e, %.2e, %.2e, %d\n" %
                            (t, opt_loss, mse_loss, tv_loss, times[-1], elapsed_time, spp))
            file_loss.flush()

            earlystopping(mse_loss)
            if earlystopping.early_stop: 
                print(f"Early Stopping! spp {spp}")
                if spp > 10000:
                    print("spp 10000 and converged, break")
                    break
                else:
                    print(f"change spp from {spp} to {spp*crspp}")
                    spp = spp * crspp
